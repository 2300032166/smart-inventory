...
async def call_groq_new(
    ...
    max_tokens = max_tokens or int(cfg.get("groq_max_tokens", os.getenv("GROQ_MAX_TOKENS", "200")))
        ...
        try:
            response = await asyncio.wait_for(
                client.chat.completions.create(
                    messages=[
                        {"role": "system", "content": "You are a professional supply chain advisor. Write EXACTLY ONE complete sentence. Never write two sentences. The sentence must end with a period. Never stop mid-sentence. Use simple English. Be direct and actionable. Vary wording per item. Never use 'Stock Risk', 'units', or repeated templates."},
