"""
Full async integration test -- replicates exact FastAPI call chain.
Run: python test_async_gemini.py
"""
import os
import sys
import asyncio
...
print(f"  GEMINI_API_KEY = {key[:6]}...{key[-4:] if key else '(EMPTY)'}")
print()

# Import the exact same modules FastAPI uses
sys.path.insert(0, os.path.dirname(__file__))

from logic.gemini_client import call_gemini
...
async def main():
    ...
    print(f"  gemini_model = {cfg.get('gemini_model')}")
    print(f"  gemini_api_key = {'SET' if cfg.get('gemini_api_key') else 'empty (will use env var)'}")
    print()

    # --- Test 1: Direct call_gemini ---
    print("=" * 50)
    print("TEST 1: Direct call_gemini()")
    print("=" * 50)
    try:
        result = await call_gemini("Say: GEMINI OK", cfg)
        print(f"PASS: {result!r}")
    except Exception as e:
        print(f"FAIL: {e}")
        print()
        print("Cannot continue -- fix call_gemini first.")
        return
    ...
    test_patterns = [
        ...
        {"avg_daily_sales": 11.46, "payday_spike": False, "declining_trend": False},
    ]
    ...
    test_reorders = [
        ...
        {"days_remaining": 4.4, "urgency": "normal",  "recommended_qty": 76,  "confidence_score": 60},
    ]
    ...
    print("SUMMARY")
    print("=" * 50)
    successes = [r for r in results if r and r != "Gemini explanation generation failed"]
    failures  = [r for r in results if not r or r == "Gemini explanation generation failed"]
    ...
    if failures:
        print()
        print("  Some SKUs failed. Check the DEBUG logs above for the exact exception.")
